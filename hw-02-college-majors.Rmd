...
**Answer:**
The majors with the lowest unemployment rate at 0.00000% are, Mathematics and Computer Science, Military Technologies, Botany, Soil Science, and Educational Administion and Supervision. Although they are all tied at 0.00000%, the dataset has Mathematics and Computer Science as number 1.
...
**Answer:**
The major with the highest percentage of women is Early Childhood Education with 96.9% of students being women.
...
**Answer:**
I used a bindwidth of 2000 because it visually balanced the size of graph with the number of bars. When using 1000 I noticed that the bars were very skinny, making the graph difficult to read. Likewise, when using a binwidth of 5000, many of the smaller bars were not visible as they were covered by other bars.

```{r fig2,height=2, eval=TRUE}
ggplot(data = college_recent_grads, mapping = aes(x = median)) +
...
**Answer:**
Based on the shape of the histogram I created, I believe that the most useful statistic for describing the distribution of data is 'Mean'. The reason for this is that the Min, Max, and Medium Median values are all not prominent on the graph. The data points hovering between 30,000 and 40,000 are the most prominent and are the clear median of the graph.
...
```{r fig3,height=2, eval=TRUE}
college_recent_grads_grouped <- college_recent_grads %>%
  group_by(major_category)

ggplot(data = college_recent_grads_grouped, aes(x = median)) +
    geom_histogram(binwidth = 2000) +
...
**Answer:**
The following table represents the highest typical (meaning highest average) median income. Engineering is the major with the highest typical median income at $57,000. This is followed by Computers & Mathematics at $45,000 and Business at $40,000.

```{r fig4,height=2, eval=TRUE}
college_recent_grads %>%
  group_by(major_category) %>%
...
**Answer:**
The following table represents the count of people in each major category ranking from least to most. The major with the lowest observation is the Interdisciplinary major with a count of 1.

```{r fig5,height=2, eval=TRUE}
college_recent_grads %>%
...
**Answer:**
The following table represents which STEM majors have median salaries equal to or less than the median for all major's earning. This table was achieved by creating the grouping 'stem_categories' and including majors such as Biology & Life Science, Computers & Mathematics, Engineering, and Physical Sciences. Next I entered the new grouping into the 'college_recent_grads' file as a new column to allow the median to be filtered.

```{r fig6, eval=TRUE, height=2}
...
**Answer:**
The following scatter plot shows the comparison of Proportion of Women in Major (along the x-axis) to the Median Income they earn (along the y-axis). Furthermore, the colour of the scatterplot point shows if they are in the stem major (blue) or not a stem major (orange). The scatterplot shows that the majors with a higher proportion of women on average have a lower median income, while the male dominated majors have a much higher average median income.

```{r fig7, eval=TRUE, height=6}
ggplot(college_recent_grads, aes(x = sharewomen, y = median, color = major_type)) +
  geom_point() +
  labs(title = "Scatterplot of Median Income vs. Proportion of Women in Major",
...
**Answer:**
My question is as follows... "What is the relationship between the proportion of women in a major and the amount of Year Round Full Time Work? Furthermore, how do the jobs in the 75th percentile contribute to this relationship?

The visualization shows that the jobs with a bigger proportion of either male or female employees are more likely to have year round full time employment. However, the jobs that have a bigger balance of men and women do not have as many year round full time employment opportunities. Moreover, the majors and jobs with more women are less likely to be a part of the 75th percentile of income.

```{r fig8, eval=TRUE, height=6}
ggplot(college_recent_grads, aes(x = sharewomen, y = employed_fulltime_yearround, color = p75th)) +
     geom_point() +
     labs(title = "Scatterplot of Proportion of Women in Major vs. Year Round Fulltime Employment",
          x = "Proportion of Women",
          y = "Year Round Full Time Employment",
          color = "75th Percentile Income") +
     scale_color_gradient(low = "coral", high = "green")
```
